...
VENDOR_MACRO = PROJECT / "packages/fiji-2017/Fiji.app/plugins/DiameterJ/DiameterJ_1-018.ijm"
...
'''
    ...
    if prompt not in generated:
        raise RuntimeError("Unsupported DiameterJ macro: source-directory prompt not found")
    ...
    if pore_marker not in generated:
        raise RuntimeError("Unsupported DiameterJ macro: pore-analysis marker not found")
...
def run_diameterj(
    ...
    selected = list(images) if images is not None else sorted(
    ...
    try:
            ...
            try:
                while not all(path.is_file() and path.stat().st_size > 0 for path in expected):
                    ...
                    if time.monotonic() >= deadline:
                        raise TimeoutError("DiameterJ timed out before all expected outputs were written")
                    time.sleep(0.5)
                time.sleep(2.0)
            finally:
        ...
        missing = [
            image.stem for image in work_images
            if not (output_dir / f"{image.stem}_Compare.png").is_file()
        ]
        if missing:
            raise SystemExit("DiameterJ did not create expected comparison output for: " + ", ".join(missing))
...
def main() -> None:
    ...
    p.add_argument("--pixel-size-um", type=float, help="µm per pixel; omit for pixel output")
